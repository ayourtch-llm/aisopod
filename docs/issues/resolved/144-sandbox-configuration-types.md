...
- Created `SandboxConfig`, `SandboxRuntime`, and `WorkspaceAccess` types in `crates/aisopod-config/src/types/sandbox.rs`
- Added `humantime` and `humantime-serde` dependencies to `aisopod-config`
- Extended `AgentBinding` with optional `sandbox: Option<SandboxConfig>` field
- Re-exported types from `aisopod-tools` via `crates/aisopod-tools/src/sandbox/`
