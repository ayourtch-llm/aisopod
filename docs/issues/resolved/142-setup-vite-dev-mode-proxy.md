...
1. **Updated `ui/vite.config.ts`**: Added proxy configuration for `/api` and `/ws` routes to proxy requests to `http://localhost:18789` (the gateway port). This enables hot module replacement (HMR) with API/WebSocket proxying during development.

2. **Updated `crates/aisopod-gateway/build.rs`**: Changed from `NO_BUILD_UI` (skip when set) to `AISOPOD_BUILD_UI` (only build when set). This makes the default behavior skip UI build for faster development cycles. Production builds require `AISOPOD_BUILD_UI=1`.

3. **Created `docs/development.md`**: Comprehensive documentation covering:
   - Prerequisites (Rust, Node.js, pnpm)
   - Production vs development build workflows
   - Hot reload development setup with step-by-step instructions
   - Testing commands
   - Code style guidelines
   - Directory structure

4. **Created `docs/learnings/142-setup-vite-dev-mode-proxy.md`**: Documentation of implementation decisions and learnings.
