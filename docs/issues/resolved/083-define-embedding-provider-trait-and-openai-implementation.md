...
1. Created `crates/aisopod-memory/src/embedding/openai.rs` with:
   - `EmbeddingProvider` trait defined with `embed()`, `embed_batch()`, and `dimensions()` methods
   - `OpenAiEmbeddingProvider` struct with `api_key`, `model`, `dimensions`, `client`, and `cache` fields
   - `new()` constructor with defaults: `model="text-embedding-3-small"`, `dimensions=1536`
   - `embed()` implementation with cache lookup, OpenAI API call, and cache storage
   - `embed_batch()` implementation with per-text cache lookup and single API call for cache misses

2. Updated `crates/aisopod-memory/src/embedding.rs` to be a module re-export file

3. Added `reqwest = { version = "0.12", features = ["json"] }` to `Cargo.toml`

4. Re-exported `EmbeddingProvider` and `OpenAiEmbeddingProvider` from `lib.rs`

5. Verified compilation with `cargo check -p aisopod-memory` and `cargo build -p aisopod-memory` both succeeding
