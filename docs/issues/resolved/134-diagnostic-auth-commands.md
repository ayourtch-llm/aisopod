...
## Acceptance Criteria
- [x] `aisopod doctor` runs all diagnostic checks and reports pass/fail
- [x] `aisopod doctor --verbose` runs extended diagnostics including network checks
- [x] Exit code is non-zero when any diagnostic fails
- [x] `aisopod auth setup` walks through provider selection and key configuration
- [x] `aisopod auth status` displays the auth state of all providers
- [x] API keys are stored securely and never displayed in plain text
- [x] Helpful URLs and instructions are shown during auth setup

## Resolution

The `aisopod doctor` and `aisopod auth` commands are now fully implemented:

### Changes Made:

1. **doctor.rs** (`crates/aisopod/src/commands/doctor.rs`):
   - Fixed to check `config.auth.profiles` instead of `config.models.providers`
   - Uses `profile.api_key.expose()` to securely access API keys
   - Added exit code handling: exits with code 1 when any diagnostic fails
   - Improved gateway health check to verify HTTP 2xx status

2. **auth.rs** (`crates/aisopod/src/commands/auth.rs`):
   - Uses `Sensitive<String>` for secure API key storage via `AuthProfile`
   - API keys are never displayed in plain text
   - Interactive setup wizard guides users with provider-specific URLs
   - Status command displays provider status without exposing keys

3. **types/auth.rs** (`crates/aisopod-config/src/types/auth.rs`):
   - Added `endpoint: Option<String>` field to `AuthProfile` struct
   - API keys stored as `Sensitive<String>` for secure handling
...
- ✅ `cargo build` passes
- ✅ `cargo test` passes (33 tests in aisopod, 55+ in aisopod-config)
