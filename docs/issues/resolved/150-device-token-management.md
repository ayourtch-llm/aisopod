...
1. **Created `crates/aisopod-gateway/src/auth/device_tokens.rs`** with:
   - `DeviceToken` struct with token_hash, device_name, device_id, scopes, timestamps, and revoked status
   - `DeviceTokenInfo` struct for metadata exposure without hashes
   - `DeviceTokenManager` struct with thread-safe Mutex-wrapped HashMap storage
   - `issue()` method to generate new tokens with argon2id hashed plaintext
   - `validate()` method to verify tokens and track last_used timestamps
   - `revoke()` method to invalidate tokens by device_id
   - `refresh()` method to generate new tokens while invalidating old ones
   - `list()` method returning DeviceTokenInfo without exposing hashes
   - `load()` and `save()` methods for persistent storage in TOML format

2. **Added routes to `crates/aisopod-gateway/src/routes.rs`**:
   - `GET /device-tokens` - list all device tokens
   - `POST /device-tokens` - issue new device token
   - `POST /device-tokens/revoke` - revoke a device token
   - `POST /device-tokens/refresh` - refresh a device token

3. **Test infrastructure fixes**:
   - Added `TestManagerWithTemp` struct with Deref/DerefMut implementations to fix test ownership issues with TempDir lifetime

4. **Comprehensive unit tests** covering:
   - Token issuance and validation
   - Token revocation preventing validation
   - Token refresh invalidating old tokens
   - Listing devices with proper scope tracking
   - Nonexistent device handling
   - Multiple devices with different scopes
   - Token hash not exposed in list output
