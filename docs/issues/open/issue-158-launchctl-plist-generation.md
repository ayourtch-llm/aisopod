...
## Acceptance Criteria
- [ ] `aisopod daemon install` generates a valid plist file on macOS
- [ ] Plist is installed to `~/Library/LaunchAgents/com.aisopod.gateway.plist`
- [ ] Label is set to `com.aisopod.gateway`
- [ ] `RunAtLoad` and `KeepAlive` are both `true`
- [ ] Log directory `~/Library/Logs/aisopod/` is created
- [ ] Stdout and stderr are directed to log files
- [ ] `launchctl load` successfully loads the service
- [ ] `aisopod daemon uninstall` unloads and removes the plist
