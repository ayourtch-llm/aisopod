...
- [ ] Auth token is stored with restrictive file permissions
- [ ] Auth token is automatically loaded and sent on connection
- [ ] Unit tests pass for all argument parsing scenarios
- [ ] Integration tests verify `--help` and `--version` output
- [ ] Integration tests verify basic command execution
