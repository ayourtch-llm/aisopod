...
## Acceptance Criteria
- [ ] `POST /v1/chat/completions` returns HTTP 501
- [ ] `POST /v1/responses` returns HTTP 501
- [ ] `POST /hooks` returns HTTP 501
- [ ] `GET /tools/invoke` returns HTTP 501
- [ ] `GET /status` returns HTTP 501
- [ ] Each request logs method, path, and status code at `info` level
- [ ] Existing `/health` endpoint is unaffected
- [ ] `cargo test -p aisopod-gateway` passes
