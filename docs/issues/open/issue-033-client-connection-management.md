...
## Acceptance Criteria
- [ ] `GatewayClient` struct contains all specified fields.
- [ ] `ClientRegistry` correctly tracks connect and disconnect events.
- [ ] `list()` returns all currently connected clients.
- [ ] `health_snapshot()` returns accurate counts.
- [ ] Disconnected clients are fully removed from the registry.
- [ ] Unit tests cover connect, disconnect, and concurrent access scenarios.
