...
## Acceptance Criteria
- [ ] `crates/aisopod-client` crate exists and compiles
- [ ] Client connects to a running aisopod server via WebSocket
- [ ] Handshake sends correct upgrade headers (Authorization, X-Aisopod-Client, etc.)
- [ ] Client receives and parses the welcome message
- [ ] `request()` sends JSON-RPC and correctly matches response by ID
- [ ] Server events are received and dispatched to the event channel
- [ ] Helper methods exist for chat, node pairing, node describe, and node invoke
- [ ] Basic integration test demonstrates a connect → authenticate → request → disconnect flow
