...
## Acceptance Criteria
- [ ] `node.pair.request` returns a pairing code and expiration time
- [ ] `node.pair.confirm` with valid code returns a device token
- [ ] `node.pair.confirm` with expired/invalid code returns an error
- [ ] `node.pair.revoke` invalidates a previously paired device
- [ ] Pairing codes expire after the configured timeout
- [ ] Paired devices are persisted across server restarts
