...
export async function handleWhatsAppStart(host: AisopodApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AisopodApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AisopodApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AisopodApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AisopodApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AisopodApp): string {
...
function resolveGatewayHttpAuthHeader(host: AisopodApp): string | null {
...
function buildGatewayHttpHeaders(host: AisopodApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AisopodApp) {
...
export function handleNostrProfileToggleAdvanced(host: AisopodApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AisopodApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AisopodApp) {
  const state = host.nostrProfileFormState;
