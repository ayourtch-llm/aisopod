...
import { AisopodApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = AisopodApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("aisopod-app") as AisopodApp;
...
beforeEach(() => {
  AisopodApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__AISOPOD_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  AisopodApp.prototype.connect = originalConnect;
  window.__AISOPOD_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/aisopod/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/aisopod");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/aisopod/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AISOPOD_CONTROL_UI_BASE_PATH__ = "/aisopod";
    const app = mountApp("/aisopod/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/aisopod");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/aisopod/sessions");
  });
