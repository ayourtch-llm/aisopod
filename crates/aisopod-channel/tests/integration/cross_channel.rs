...
use crate::integration::mock_servers::{SlackMockState, DiscordMockState, TestServer};
...
async fn test_cross_channel_rate_limit_slack_vs_discord() {
    ...
    assert_eq!(slack_config.global_limit.window_duration, Duration::from_secs(1));
    ...
    assert_eq!(discord_config.global_limit.window_duration, Duration::from_secs(5));
}
...
async fn test_cross_channel_media_validation() {
    ...
    use aisopod_channel::util::media::{MediaAttachment, MediaFormat, Platform, ImageFormat};
    ...
    let platforms = [Platform::Telegram, Platform::Discord, Platform::Slack, Platform::WhatsApp];
...
async fn test_cross_channel_message_formatting() {
    ...
    use aisopod_channel::util::formatting::{NormalizedMarkdown, from_plain_text};
...
async fn test_cross_channel_server_isolation() {
    ...
    let mock_server1 = crate::integration::mock_servers::create_slack_mock_server(Default::default()).await;
    let server1 = TestServer::from_wiremock(mock_server1);
    
    let mock_server2 = crate::integration::mock_servers::create_slack_mock_server(Default::default()).await;
