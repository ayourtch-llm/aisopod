...
impl NostrKeys {
    ...
    pub fn from_nsec(nsec: &str) -> Result<Self, KeyError> {
        ...
        let data_u8: Vec<u8> = data.into_iter().map(|b| b.to_u8()).collect();
    ...
    pub fn from_pubkey_hex(pubkey_hex: &str) -> Result<Self, KeyError> {
        let secp = Secp256k1::<All>::new();
        let bytes = hex::decode(pubkey_hex).map_err(|e| KeyError::InvalidHex(e.to_string()))?;
        let public_key =
            PublicKey::from_slice(&bytes).map_err(|e| KeyError::InvalidPublicKey(e.to_string()))?;

        // Use a dummy secret key for read-only operations
        let secret_key =
            SecretKey::from_slice(&[0u8; 32]).map_err(|e| KeyError::InvalidHex(e.to_string()))?;
    ...
    pub fn npub(&self) -> String {
        ...
        let data_base32: Vec<bech32::u5> = data_bytes
            .iter()
            .map(|b| bech32::u5::try_from_u8(*b).expect("value < 256"))
            .collect();
        encode("npub", &data_base32, Variant::Bech32).expect("bech32 encoding should succeed")
    }
    ...
    pub fn sign(&self, msg: &[u8]) -> Result<String, KeyError> {
        ...
        let msg_hash = Message::from_slice(msg).map_err(|e| KeyError::InvalidHex(e.to_string()))?;
