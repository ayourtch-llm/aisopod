...
impl BedrockProvider {
    ...
    pub async fn new(
        ...
        let default_model =
            default_model.unwrap_or_else(|| "anthropic.claude-3-sonnet-20240229-v1:0".to_string());
    ...
    fn get_api_key(&self) -> Option<String> {
        let mut manager = self.profile_manager.lock().unwrap();
        manager.next_key("bedrock").map(|p| p.api_key.clone())
    }
    ...
    fn build_bedrock_request(&self, request: &ChatCompletionRequest) -> Result<BedrockRequest> {
        ...
        let system = self
            .extract_system_prompt(&request.messages)
    ...
    fn parse_stream_event(event: &BedrockStreamEvent) -> Option<ChatCompletionChunk> {
        match event {
            BedrockStreamEvent::ContentBlockStart {
                ...
                if let BedrockContentBlock::ToolUse {
                    tool_use_id, name, ..
                } = content_block
                {
            ...
            BedrockStreamEvent::MessageStart { message, .. } => {
                ...
                Some(ChatCompletionChunk {
                    id: message.id.clone(),
                    delta: MessageDelta {
                        role: message.role.as_ref().map(|r| match r {
                            BedrockRole::User => Role::User,
                            BedrockRole::Assistant => Role::Assistant,
                        }),
                        content: None,
                        tool_calls: None,
                    },
    ...
    fn map_aws_error(&self, error: anyhow::Error) -> anyhow::Error {
        ...
        if error.to_string().contains("AccessDenied") || error.to_string().contains("Unauthorized")
        {
            return anyhow::anyhow!("AWS access denied: {}", error);
        }

        // Check for model timeout
        if error.to_string().contains("ModelTimeout") || error.to_string().contains("Timeout") {
...
mod tests {
    ...
    fn test_bedrock_content_block_serialization() {
        let text_block = BedrockContentBlock::Text {
