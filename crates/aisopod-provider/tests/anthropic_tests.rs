...
#[tokio::test]
async fn test_anthropic_provider_with_temperature() {
    let provider = AnthropicProvider::new(
        "test-key".to_string(),
        None,
        Some(0.7),
...
#[tokio::test]
async fn test_anthropic_provider_with_max_tokens() {
    let provider = AnthropicProvider::new(
        "test-key".to_string(),
        None,
        None,
...
fn test_anthropic_sse_message_start_event() {
    ...
    if let Ok(event) = result {
        if let anthropic_api::AnthropicSseEvent::MessageStart { message } = event {
            ...
            assert_eq!(message.role, anthropic_api::AnthropicRole::Assistant);
        }
    }
}
...
fn test_build_anthropic_request_with_system_prompt() {
    let request = anthropic_api::AnthropicRequest {
        ...
        system: Some(vec![anthropic_api::AnthropicContentBlock::Text {
...
fn test_parse_anthropic_stream_response() {
    ...
    if let Ok(response) = result {
        ...
        assert_eq!(response.usage.map(|u| u.input_tokens), Some(10));
        assert_eq!(response.usage.map(|u| u.output_tokens), Some(5));
    }
}
...
#[test]
fn test_anthropic_provider_with_all_options() {
    let provider = AnthropicProvider::new(
        "test-key".to_string(),
        Some("org-123".to_string()),
        Some(0.7),
...
fn test_anthropic_image_source_roundtrip() {
    ...
    assert_eq!(parsed.type_field, "base64");
...
fn test_anthropic_error_response_roundtrip() {
    ...
    assert_eq!(parsed.error_type, original.error_type);
...
fn test_anthropic_content_block_delta() {
    let delta = anthropic_api::AnthropicContentBlockDelta {
    ...
    assert_eq!(parsed.type_field, "text_delta");
...
fn test_anthropic_stream_event_type() {
    let event = anthropic_api::AnthropicSseEvent::ContentBlockDelta {
        index: 0,
        delta: anthropic_api::AnthropicContentBlockDelta {
            r#type: "text_delta".to_string(),
            text: "Hello".to_string(),
        },
    };
...
// ============================================================================
// Provider Creation with All Parameters Tests
// ============================================================================

#[test]
fn test_anthropic_provider_with_all_parameters() {
    let provider = AnthropicProvider::new(
        "test-key".to_string(),
        Some("org-123".to_string()),
        Some(0.7),
