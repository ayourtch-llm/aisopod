...
/// * `client_ip` - The IP address of the client (for audit logging)
///
/// # Returns
///
...
pub fn check_scope(auth_info: &AuthInfo, method: &str, client_ip: &str) -> Result<(), RpcResponse> {
...
mod tests {
    ...
    fn test_check_scope_no_required_scope() {
        ...
        assert!(check_scope(&auth_info, "unknown.method", "127.0.0.1").is_ok());
    }

    #[test]
    fn test_check_scope_with_matching_scope() {
        let auth_info = auth_info_with_scopes(vec!["operator.read"]);
        assert!(check_scope(&auth_info, "agent.list", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "session.get", "127.0.0.1").is_ok());
    }
    ...
    fn test_check_scope_admin_allows_all() {
        ...
        assert!(check_scope(&auth_info, "agent.list", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "agent.start", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "admin.shutdown", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "approval.approve", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "pairing.initiate", "127.0.0.1").is_ok());
    }
    ...
    fn test_check_scope_write_allows_write_methods() {
        ...
        assert!(check_scope(&auth_info, "agent.start", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "chat.send", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "config.update", "127.0.0.1").is_ok());
    }

    #[test]
    fn test_check_scope_write_denies_admin() {
        let auth_info = auth_info_with_scopes(vec!["operator.write"]);
        let result = check_scope(&auth_info, "admin.shutdown", "127.0.0.1");
        assert!(result.is_err());
    }
    ...
    fn test_check_scope_approvals() {
        ...
        assert!(check_scope(&auth_info, "approval.request", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "approval.approve", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "approval.deny", "127.0.0.1").is_ok());
    }
    ...
    fn test_check_scope_pairing() {
        ...
        assert!(check_scope(&auth_info, "pairing.initiate", "127.0.0.1").is_ok());
        assert!(check_scope(&auth_info, "pairing.confirm", "127.0.0.1").is_ok());
    }
    ...
    fn test_check_scope_empty_scopes() {
        ...
        let result = check_scope(&auth_info, "agent.list", "127.0.0.1");
        assert!(result.is_err());
    }
}
