...
use aisopod_gateway::static_files::{static_handler, StaticFileState};
...
async fn test_serve_hashed_asset() {
    ...
    assert!(
        cache_val.contains("immutable"),
        "Expected immutable cache: {}",
        cache_val
    );
    assert!(
        cache_val.contains("max-age=31536000"),
