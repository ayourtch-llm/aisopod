...
fn generate_header(format: ConfigFormat) -> &'static str {
    match format {
        ConfigFormat::Json5 => "// Aisopod default configuration\n\
                                // Edit this file to customize your setup.\n\
                                // See documentation for all available options.\n\n",
        ConfigFormat::Toml => "# Aisopod default configuration\n\
                               # Edit this file to customize your setup.\n\
                               # See documentation for all available options.\n\n",
    }
}
...
mod tests {
    ...
    fn test_default_values_match() {
        ...
        assert_eq!(config.session.messages.max_messages, defaults.session.messages.max_messages);
        assert_eq!(config.tools.bash.enabled, defaults.tools.bash.enabled);
        assert_eq!(config.skills.settings.timeout, defaults.skills.settings.timeout);
    }
    ...
    fn test_generated_configs_are_valid() {
        ...
        assert_eq!(json5_config.gateway.server.port, toml_config.gateway.server.port);
    }
}
