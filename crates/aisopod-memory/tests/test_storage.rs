...
use aisopod_memory::{
    MemoryEntry, MemoryFilter, MemoryMetadata, MemorySource, MemoryStore,
};
...
async fn test_delete_entry() {
    ...
    let entry = make_entry("delete-id", "agent-1", "To be deleted", &[0.1, 0.2, 0.3, 0.4]);
...
async fn test_list_with_tag_filter() {
    ...
    for entry in &all_entries {
        eprintln!("  Entry: id={}, content={}, tags={:?}", entry.id, entry.content, entry.metadata.tags);
    }
...
async fn test_store_overwrites_existing() {
    ...
    // Create and store an entry
    let entry1 = make_entry("same-id", "agent-1", "Original content", &[0.1, 0.2, 0.3, 0.4]);
    ...
    let entry2 = make_entry("same-id", "agent-1", "Updated content", &[0.5, 0.6, 0.7, 0.8]);
