...
fn build_conversation(
    ...
    assistant_messages: Vec<&str>,
) -> Vec<aisopod_provider::types::Message> {
    ...
    for i in 0..min_len {
        ...
        messages.push(aisopod_provider::types::Message {
            ...
            content: aisopod_provider::types::MessageContent::Text(
                assistant_messages[i].to_string(),
            ),
            tool_calls: None,
            tool_call_id: None,
        });
...
async fn store_test_memories(pipeline: &MemoryQueryPipeline, agent_id: &str, memories: Vec<&str>) {
...
async fn memory_tool_delete(pipeline: &MemoryQueryPipeline, id: &str) -> Result<(), anyhow::Error> {
...
async fn test_build_memory_context() {
    ...
    store_test_memories(
        &pipeline,
        agent_id,
        vec!["User prefers vegetarian food", "User likes Italian cuisine"],
    )
    .await;
...
async fn test_build_memory_context_empty() {
    ...
    let agent_id = "agent-1";
    let conversation = build_conversation(vec!["Hello"], vec!["Hi there! How can I help you?"]);
...
async fn test_no_memory_configured() {
    ...
    impl aisopod_memory::MemoryStore for MockStore {
        async fn store(
            &self,
            _entry: aisopod_memory::MemoryEntry,
        ) -> Result<String, anyhow::Error> {
...
async fn test_memory_manager_with_integration() {
    ...
    let entries = manager
        .extract_memories(agent_id, &conversation)
        .await
        .unwrap();
    ...
    let matches = pipeline
        .query("food preferences", MemoryQueryOptions::default())
        .await
        .unwrap();
...
async fn test_memory_context_with_message_parts() {
    ...
    let conversation = vec![
        ...
        aisopod_provider::types::Message {
            role: aisopod_provider::types::Role::Assistant,
            content: aisopod_provider::types::MessageContent::Text(
                "I can recommend some spicy dishes!".to_string(),
            ),
            tool_calls: None,
            tool_call_id: None,
        },
